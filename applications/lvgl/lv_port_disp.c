...
static void nu_fill_cb(struct _lv_disp_drv_t* disp_drv,
                       lv_color_t* dest_buf,
                       ...
                       lv_color_t color)
{
    ...
    if (lv_area_get_size(fill_area) < 3600) {
        ...
        // rt_kprintf("[blend_fill %d %08x] %dx%d %d %d\n", lv_area_get_size(fill_area), dest_buf, fill_area_w,
        // fill_area_h, fill_area->x1, fill_area->y1 );

        if (IS_CACHEABLE_VRAM(dest_buf))
            mmu_clean_invalidated_dcache((uint32_t)dest_buf,
                                         sizeof(lv_color_t) * (dest_width * fill_area_h + fill_area_w));
...
void lv_port_disp_init(void)
{
    ...
    RT_ASSERT(info.bits_per_pixel == 8 || info.bits_per_pixel == 16 || info.bits_per_pixel == 24 ||

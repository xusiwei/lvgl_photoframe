...
static void lv_draw_n9h30_ge2d_blend_fill(lv_color_t* dest_buf,
                                          lv_coord_t dest_stride,
                                          const lv_area_t* fill_area,
                                          lv_color_t color);

static void lv_draw_n9h30_ge2d_blend_map(lv_color_t* dest_buf,
...
void lv_draw_n9h30_ge2d_blend(lv_draw_ctx_t* draw_ctx, const lv_draw_sw_blend_dsc_t* dsc)
{
    ...
    if ((lv_area_get_size(&blend_area) > 7200) && (((blend_area_w * (LV_COLOR_DEPTH / 2)) & 0x3) == 0) &&
                                          ...
                                          lv_color_t color)
{
    ...
    // rt_kprintf("[blend_fill %d %08x] %dx%d %d %d\n", lv_area_get_size(fill_area), dest_buf, fill_area_w, fill_area_h,
    // fill_area->x1, fill_area->y1 );

    if (IS_CACHEABLE_VRAM(dest_buf))
        mmu_clean_invalidated_dcache((uint32_t)dest_buf,
                                     sizeof(lv_color_t) * (dest_stride * fill_area_h + fill_area_w));
